...
private:
  ...
  void publish(const INSData & d)
  {
    ...
    st.heading_std = static_cast<float>(d.yaw_std); st.rtk_fixed = d.position_type == 4; status_->publish(st);
  }
  std::string port_, frame_id_; int baudrate_{}; SerialPort serial_; ASENSINGParser parser_; rclcpp::TimerBase::SharedPtr timer_;
  rclcpp::Publisher<sensor_msgs::msg::NavSatFix>::SharedPtr fix_; rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr pose_;
  rclcpp::Publisher<geometry_msgs::msg::TwistStamped>::SharedPtr velocity_; rclcpp::Publisher<INSStatus>::SharedPtr status_;
};
}
